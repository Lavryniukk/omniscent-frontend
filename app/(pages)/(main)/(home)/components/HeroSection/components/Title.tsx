...
let Title = () => {
  ...
  const [slidingAnimation, slidingAnimationParent] = useSlideIn();

  return (
    <motion.div
      {...slidingAnimationParent}
    ...
    >
     ...
     your patience.
      ...
      <motion.div
        {...slidingAnimation}
        className="mx-auto group duration-500 border relative xl:p-6 lg:p-5 p-4 w-fit h-fit flex items-center justify-center rounded-xl border-secondary bg-background"
      >
        {theme && theme == "light" ? (
          <Image
        ...
        <motion.div
          transition={{ type: spring, duration: 0.8, delay: 1.4 }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="absolute group-hover:animate-pulse rounded-full transition-all duration-500 delay-500 bg-gradient-to-r from-primary/80 to-primary/60 aspect-square xl:w-40 lg:w-36 w-32 -z-10 blur-2xl"
        />
      </motion.div>
      <>
        <motion.h2
          {...slidingAnimation}
          className="text-xl text-center mx-auto sm:text-2xl text-accent pl-1 tracking-[5px] font-extrabold flex justify-center duration-500 w-fit"
        >
          CLEVERIZE
        </motion.h2>
        ...
        <motion.h1
          {...slidingAnimation}
          className=" transition duration-500 bg-gradient-to-t from-text to-text/70 text-transparent w-full md:w-10/12 antialiased mx-auto leading-none
      ...
      <motion.p
        {...slidingAnimation}
        className=" mb-6 transition delay-100 duration-500 text-accent hyphens-manual mx-auto flex items-center justify-center w-10/12 md:w-3/4 lg:w-1/2 text-md sm:text-xl text-center "
      >
